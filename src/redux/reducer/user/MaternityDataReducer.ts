...
export const fetchUserMaternityData = createAsyncThunk<
string[], 
  {
    page?: number;
    ...
    user_id?: number;
      ...
      'sanjeeviniApp/fetchUserMaternityData',
      async (args:any, thunkAPI:any) => {
        try {
          let url = API_URL + 'usermaternityapp/?page=';
          if (args.page) {
            url += args.page;
          }
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder:any) => {
    builder
        .addCase(fetchUserMaternityData.pending, (state:any) => {
          state.loading = true;
          state.error = null;
        })
        .addCase(fetchUserMaternityData.fulfilled, (state:any, action:any) => {
          state.loading = false;
          state.userData.push(...action.payload);
          state.totalPages = action.meta.arg.total_pages;
        })
        .addCase(fetchUserMaternityData.rejected, (state:any, action:any) => {
          state.loading = false;
