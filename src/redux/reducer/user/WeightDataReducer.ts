...
export const fetchUserWeightData = createAsyncThunk<
string[], 
  {
    page?: number;
    ...
    user_id?: number;
      ...
      'sanjeeviniApp/fetchUserWeightData',
      async (args:any, thunkAPI:any) => {
        try {
          let url = API_URL + 'userweightapp/?page=';
          if (args.page) {
            url += args.page;
          }
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder:any) => {
    builder
        .addCase(fetchUserWeightData.pending, (state:any) => {
          state.loading = true;
          state.error = null;
        })
        .addCase(fetchUserWeightData.fulfilled, (state:any, action:any) => {
          state.loading = false;
          state.userData.push(...action.payload);
          state.totalPages = action.meta.arg.total_pages;
        })
        .addCase(fetchUserWeightData.rejected, (state:any, action:any) => {
          state.loading = false;
